...
def save_notification_to_db(user_id: int, user_email: str, message: str, 
    ...
    try:
        ...
        notification = Notification(
            ...
            title=title,  # TÍTULO AÑADIDO
            is_read=False
        )
        db.add(notification)
        db.commit()
        db.refresh(notification)
        logger.info(f"✅ Notificación guardada en BD: ID={notification.id}, Email={user_email}, Título='{title}'")
        return notification
    except Exception as e:
        db.rollback()
    ...
    try:
        ...
        if event_type == "APPOINTMENT_CREATED" or "APPOINTMENT_CREATED" in str(message_data):
            ...
            if doctor_id and appointment_date:
                ...
                title = f"Cita médica con Doctor {doctor_id}"
            else:
            ...
            title = f"Cita cancelada #{appointment_id}"
            ...
            title = f"Evento del sistema: {event_type}"
