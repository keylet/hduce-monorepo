...
class SimpleRabbitMQConsumer:
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"✅ Consumer conectado a RabbitMQ")
            return True
    ...
    def handle_appointment_created(self, appointment_data: Dict[str, Any]):
        ...
        try:
            ...
            print(f"📅 Nueva cita: ID={appointment_id}, Paciente={patient_id}")
            ...
            print(f"✅ Notificación creada para cita {appointment_id}")
        ...
        def consumer_loop():
            while True:
                try:
                    ...
                    print(f"❌ Consumer detenido, reintentando en 5s: {e}")
                    import time
                    time.sleep(5)
        
        thread = threading.Thread(target=consumer_loop, daemon=True)
        thread.start()
        print("✅ Consumer iniciado en background")
        return thread
...
def start_rabbitmq_consumer():
    ...
    try:
        ...
        print("🚀 Iniciando RabbitMQ consumer simplificado...")
        thread = consumer.start_in_background()
        if thread:
            print("✅ Consumer simplificado iniciado exitosamente")
            return consumer
        else:
            print("❌ No se pudo iniciar consumer")
            return None
    except Exception as e:
        print(f"❌ Error iniciando consumer: {e}")
        import traceback
