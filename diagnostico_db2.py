...
print(f"3. URL de conexión REAL (con password): {str(engine.url)}")
...
try:
    ...
    if current_db != "appointment_db":
        print(f"❌ ERROR CRÍTICO: Conectado a {current_db} en lugar de appointment_db")
    else:
        print(f"✅ CORRECTO: Conectado a {current_db}")
    
    # Verificar todos los doctores en esta conexión
    result = db.execute(text("SELECT id, name FROM doctors ORDER BY id LIMIT 5"))
